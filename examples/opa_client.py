...
class OpaClient:
    ...
    def __init__(self):
        ...
        try:
            ...
            raise OpaConnectionError(
                f"Failed to connect to OPA server during initialization at {self.base_url}: {str(e)}"
            ) from e
    ...
    def verify_connection(self) -> None:
        ...
        try:
            ...
            raise OpaConnectionError(
                f"Failed to connect to OPA server health endpoint at {health_url}: {str(e)}"
            ) from e
        except Exception as e:
            # Catch other potential errors during request or status check
            raise OpaConnectionError(
                f"An unexpected error occurred while verifying connection to {health_url}: {str(e)}"
            ) from e
    ...
    def load_policy(self, policy_name: str, policy_content: str) -> None:
        ...
        try:
            response = requests.put(
                url, data=policy_content.encode("utf-8"), headers=headers, timeout=10
            ...
            raise OpaPolicyError(
                f"Failed to load policy '{policy_name}' to {url}. {error_details}"
            ) from e
        except Exception as e:
            raise OpaPolicyError(
                f"An unexpected error occurred loading policy '{policy_name}': {str(e)}"
            ) from e
    ...
    def load_policy_from_file(self, policy_path: str) -> None:
        ...
        try:
            ...
            if not policy_name:
                raise OpaPolicyError(
                    f"Could not derive a valid policy name from path: {policy_path}"
                )
            ...
            raise OpaPolicyError(
                f"Error reading or loading policy from file {policy_path}: {str(e)}"
            ) from e
    ...
    def load_data(self, path: str, data: Any) -> None:
        ...
        try:
            ...
            raise TypeError(
                f"Data for path '{path}' is not JSON serializable: {str(e)}"
            ) from e

        try:
            response = requests.put(
                url, data=json_data.encode("utf-8"), headers=headers, timeout=10
            )
            response.raise_for_status()  # Checks for 4xx/5xx errors
            # Status 204 No Content is also a success for PUT /v1/data
            print(
                f"Successfully loaded data at path '{path}' (Status: {response.status_code})"
            )
        except RequestException as e:
            error_details = f"Network error: {str(e)}"
            if hasattr(e, "response") and e.response is not None:
            ...
            raise OpaClientError(
                f"Failed to load data to path '{path}' at {url}. {error_details}"
            ) from e
        except Exception as e:
            raise OpaClientError(
                f"An unexpected error occurred loading data to '{path}': {str(e)}"
            ) from e

    def evaluate_policy(
        self, policy_query: str, input_data: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        """
        ...
        if not policy_query.startswith("/"):
            policy_query = "/" + policy_query.replace(
                ".", "/"
        ...
        if input_data is not None:
            try:
                ...
                raise TypeError(
                    f"Input data for query '{policy_query}' is not JSON serializable: {str(e)}"
                ) from e
        ...
        try:
            ...
            if request_body:
                response = requests.post(
                    url, data=request_body, headers=headers, timeout=10
                )
            else:
            ...
            try:
                ...
                raise OpaPolicyError(
                    f"OPA server returned non-JSON response for query '{policy_query}'. Status: {response.status_code}. Response text: {response.text[:1000]}"
                ) from e
            ...
            if status_code == 404:
                raise OpaPolicyError(
                    f"Policy or data not found at query path '{policy_query}' (404). URL: {url}. {error_details}",
                    404,
                ) from e
            else:
                raise OpaPolicyError(
                    f"Policy evaluation failed for query '{policy_query}'. {error_details}",
                    status_code,
                ) from e
        except Exception as e:
            raise OpaPolicyError(
                f"An unexpected error occurred during policy evaluation for '{policy_query}': {str(e)}"
            ) from e
...
if __name__ == "__main__":
    try:
        ...
        )  # Load data under 'permissions' path
        ...
        print(
            f"Test Case 1 (action='write', role='editor', perms=['read_access', 'write_access']) -> allow: {result_1}"
        )
        assert result_1 is True

        # --- Test Case 2: User without correct permission ---
        test_input_2 = {
            "user": {"id": "user2", "role": "viewer", "permissions": ["read_access"]},
            "action": "write",  # Requires 'write_access'
        }
        result_2 = client.evaluate_policy("data.example.allow", test_input_2)
        print(
            f"Test Case 2 (action='write', role='viewer', perms=['read_access']) -> allow: {result_2}"
        )
        ...
        retrieved_data = client.evaluate_policy(
            "data.permissions"
    ...
    # Define test input
    test_input = {
        "user": {"id": "user1", "role": "regular", "permissions": ["read", "write"]},
        "action": "write",
    }
