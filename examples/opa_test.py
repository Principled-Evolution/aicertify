...
def check_shutil_which(name: str) -> None:
    ...
    if result:
        ...
        try:
            os_result = subprocess.run([result, "version"], capture_output=True, text=True, timeout=5)
...
def main() -> None:
    ...
    try:
        result = subprocess.run(["opa.exe", "version"],
                                capture_output=True,
                                text=True,
                                timeout=5)
        if result.returncode == 0:
        ...
        print("❌ 'opa.exe' was not found in the current PATH. Ensure it is installed and available.")
    except Exception as e:
    ...
    if sys.platform.startswith("linux"):
        ...
        try:
            result = subprocess.run([OPA_PATH_LINUX, "version"],
                                    capture_output=True,
                                    text=True,
                                    timeout=5)
            if result.returncode == 0:
            ...
            print(f"❌ '{OPA_PATH_LINUX}' was not found. Ensure it is installed and available.")
        except Exception as e:
