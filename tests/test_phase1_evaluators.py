...
class TestFairnessEvaluator:
    ...
    @pytest.mark.asyncio
    @patch("aicertify.evaluators.fairness_evaluator.langfair")
    async def test_fairness_evaluator_non_compliant(
        self, mock_langfair, sample_contract
    ):
...
class TestContentSafetyEvaluator:
    ...
    @pytest.mark.asyncio
    @patch("aicertify.evaluators.content_safety_evaluator.deepeval")
    async def test_content_safety_evaluator_non_compliant(
        self, mock_deepeval, sample_contract
    ):
...
class TestComplianceEvaluator:
    ...
    @patch(
        "aicertify.evaluators.content_safety_evaluator.ContentSafetyEvaluator.evaluate_async"
    )
    @patch(
        "aicertify.evaluators.risk_management_evaluator.RiskManagementEvaluator.evaluate_async"
    )
    async def test_compliance_evaluator(
        self, mock_risk, mock_safety, mock_fairness, sample_contract
    ):
    ...
    async def test_compliance_evaluator_non_compliant(
        self, mock_risk, mock_safety, mock_fairness, sample_contract
    ):
