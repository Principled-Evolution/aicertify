        ...
        "risk_factors": [
            {
                ...
                "description": patient_data.get("medical_history", ""),
            },
            {
                "type": "demographic",
                "description": f"Age: {patient_data.get('age', 0)}, Gender: {patient_data.get('gender', '')}",
            },
        ],
        "governance": {
...
def create_contract(
    context: Dict[str, Any], model_id: str, interaction_type: str
) -> Contract:
    """Create a test contract."""
    return Contract(
        context=context, model_id=model_id, interaction_type=interaction_type
    )
...
contract = create_contract(
    context=context, model_id="test-model", interaction_type="analysis"
)
...
print(
    f"Loaded context keys: {list(loaded_context.keys() if loaded_context else {}.keys())}"
)
