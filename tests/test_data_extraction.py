...
def test_create_evaluation_report_without_opa(empty_opa_results):
    ...
    eval_result = {
        "app_name": "Test App",
        "metrics": {
            "toxicity": {"name": "Toxicity", "value": 0.1, "control_passed": True}
        },
    }
...
def test_create_evaluation_report_with_opa(mock_extract):
    ...
    mock_policy_results = [
        PolicyResult(
            ...
            result=True,
            metrics={
                "metric1": {"name": "Metric 1", "value": 0.5, "control_passed": True}
            },
        ),
        PolicyResult(
            name="Test Policy 2",
            result=False,
            metrics={
                "metric2": {"name": "Metric 2", "value": 0.8, "control_passed": False}
            },
        ),
    ]
    ...
    policy1 = next(
        (p for p in report.policy_results if p.name == "Test Policy 1"), None
    )
    policy2 = next(
        (p for p in report.policy_results if p.name == "Test Policy 2"), None
    )
...
def test_integration_with_real_data():
    ...
    eval_result = {
        ...
        "metrics": {
            "test_metric": {"name": "Test Metric", "value": 1.0, "control_passed": True}
        },
    }
    ...
    has_policies = (
        opa_results
        and isinstance(opa_results.get("result"), dict)
