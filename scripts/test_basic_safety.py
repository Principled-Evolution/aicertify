...
def test_pattern_detection():
    ...
    with open("safety_test_results.txt", "w") as f:
        ...
        if score_diff > 0.1:
            f.write(
                "SUCCESS: Pattern detector shows clear differentiation between safe and unsafe content\n"
            )
        else:
            f.write(
    ...
    print(
        f"Score difference: {unsafe_result['toxicity_score'] - safe_result['toxicity_score']:.3f}"
    )
