...
def test_fairness_evaluator_ab():
    ...
    try:
        ...
        score_range = max(fair_result.score, biased_result.score, medium_result.score) - min(fair_result.score, biased_result.score, medium_result.score)
        logger.info(f"Score range (max difference): {score_range:.3f}")
        
        if score_range < 0.1:
            logger.warning("WARNING: Minimal differentiation between contrasting datasets!")
            logger.warning("This suggests the metrics may not be genuinely measuring bias.")
        else:
            logger.info("VALIDATED: Fairness metrics show meaningful differentiation between datasets")
...
def test_content_safety_evaluator_ab():
    ...
    try:
        ...
        # Check if there's meaningful differentiation
        score_range = max(safe_result.score, unsafe_result.score, medium_result.score) - min(safe_result.score, unsafe_result.score, medium_result.score)
        logger.info(f"Score range (max difference): {score_range:.3f}")
        
        if score_range < 0.1:
            logger.warning("WARNING: Minimal differentiation between contrasting datasets!")
            logger.warning("This suggests the metrics may not be genuinely measuring content safety.")
        else:
            logger.info("VALIDATED: Content safety metrics show meaningful differentiation between datasets")
...
def test_risk_management_evaluator_ab():
    ...
    try:
        ...
        fair_safe_with_context["risk_documentation"] = {
            ...
            "mitigation_measures": ["Extensive testing conducted", "Trained on balanced datasets", "Regular auditing in place"],
        ...
        medium_risk_with_context["risk_documentation"] = {
            ...
            "mitigation_measures": ["Basic content filtering", "Some diversity in training data"],
        ...
        # Check if there's meaningful differentiation
        score_range = max(low_risk_result.score, high_risk_result.score, medium_risk_result.score) - min(low_risk_result.score, high_risk_result.score, medium_risk_result.score)
        logger.info(f"Score range (max difference): {score_range:.3f}")
        
        if score_range < 0.1:
            logger.warning("WARNING: Minimal differentiation between contrasting datasets!")
            logger.warning("This suggests the metrics may not be genuinely measuring risk management.")
        else:
            logger.info("VALIDATED: Risk management metrics show meaningful differentiation between datasets")
...
def visualize_results(fairness_results, safety_results, risk_results):
    ...
    try:
        ...
        # Create bars
        ax.bar(r1, [scores[k][0] for k in scores.keys()], width, label='Good Sample', color='green')
        ax.bar(r2, [scores[k][2] for k in scores.keys()], width, label='Medium Sample', color='orange')  
        ax.bar(r3, [scores[k][1] for k in scores.keys()], width, label='Bad Sample', color='red')
...
def save_results(fairness_results, safety_results, risk_results, output_path=None):
    ...
    try:
        ...
        results = {
            "fairness": {
                "good_sample": fairness_results[0].model_dump() if fairness_results[0] else None,
                "bad_sample": fairness_results[1].model_dump() if fairness_results[1] else None,
                "medium_sample": fairness_results[2].model_dump() if fairness_results[2] else None,
            },
            "content_safety": {
                "good_sample": safety_results[0].model_dump() if safety_results[0] else None,
                "bad_sample": safety_results[1].model_dump() if safety_results[1] else None,
                "medium_sample": safety_results[2].model_dump() if safety_results[2] else None,
            },
            "risk_management": {
                "good_sample": risk_results[0].model_dump() if risk_results[0] else None,
                "bad_sample": risk_results[1].model_dump() if risk_results[1] else None,
                "medium_sample": risk_results[2].model_dump() if risk_results[2] else None,
            }
        }
        ...
        table_data = []
        table_data.append(["Evaluator", "Good Sample", "Medium Sample", "Bad Sample", "Diff (Good-Bad)"])
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run A/B tests on AICertify evaluators")
