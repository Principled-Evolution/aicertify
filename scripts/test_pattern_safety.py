...
def test_pattern_detector():
    ...
    # Display results
    logger.info(
        f"Safe text score: {safe_result['toxicity_score']:.3f}, Passed: {safe_result['passed']}"
    )
    logger.info(
        f"Medium text score: {medium_result['toxicity_score']:.3f}, Passed: {medium_result['passed']}"
    )
    logger.info(
        f"Unsafe text score: {unsafe_result['toxicity_score']:.3f}, Passed: {unsafe_result['passed']}"
    )
    logger.info(
        f"Multi-category text score: {multi_result['toxicity_score']:.3f}, Passed: {multi_result['passed']}"
    )
    ...
    table_data.append(
        [
            ...
            ", ".join(
                [c for c, s in safe_result["category_scores"].items() if s > 0.1]
            ),
        ]
    )
    ...
    table_data.append(
        [
            ...
            ", ".join(
                [c for c, s in medium_result["category_scores"].items() if s > 0.1]
            ),
        ]
    )
    ...
    table_data.append(
        [
            ...
            ", ".join(
                [c for c, s in unsafe_result["category_scores"].items() if s > 0.1]
            ),
        ]
    )
    ...
    table_data.append(
        [
            ...
            ", ".join(
                [c for c, s in multi_result["category_scores"].items() if s > 0.1]
            ),
        ]
    )
    ...
    if score_range < 0.1:
        ...
        logger.info(
            "SUCCESS: Pattern-based detector shows meaningful differentiation between texts"
        )
    ...
    safe_data = {
        "interactions": [
            {"input_text": "Tell me about France", "output_text": SAFE_TEXT}
        ]
    }
    unsafe_data = {
        "interactions": [
            {"input_text": "How are you feeling?", "output_text": UNSAFE_TEXT}
        ]
    }
    ...
    logger.info(
        f"Safe evaluation score: {safe_eval.score:.3f}, Compliant: {safe_eval.compliant}"
    )
    logger.info(
        f"Unsafe evaluation score: {unsafe_eval.score:.3f}, Compliant: {unsafe_eval.compliant}"
    )
    ...
    eval_table.append(
        [
            ...
            (
                safe_eval.reason[:60] + "..."
                if len(safe_eval.reason) > 60
                else safe_eval.reason
            ),
        ]
    )
    ...
    eval_table.append(
        [
            ...
            (
                unsafe_eval.reason[:60] + "..."
                if len(unsafe_eval.reason) > 60
                else unsafe_eval.reason
            ),
        ]
    )
    ...
    if eval_diff < 0.1:
        ...
        logger.info(
            "SUCCESS: Full evaluation shows meaningful differentiation between safe and unsafe content"
        )
