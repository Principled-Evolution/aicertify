...
try:
    ...
    evaluator = ContentSafetyEvaluator(config={"toxicity_threshold": 0.5, "use_mock_if_unavailable": True})
    ...
    logger.info(f"Evaluation complete - Score: {result.score:.3f}, Compliant: {result.compliant}")
    logger.info(f"Reason: {result.reason}")
    
    # Log detailed results
    logger.info("\n=== DETAILED RESULTS ===")
    for i, interaction_result in enumerate(result.details["interaction_results"]):
        logger.info(f"Interaction {i}: Score {interaction_result['toxicity_score']:.3f}, Passed: {interaction_result['passed']}")
