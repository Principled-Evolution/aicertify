...
def test_fairness_evaluator():
    ...
    try:
        ...
        # Print the results
        logger.info(
            f"Evaluation complete. Compliant: {result.compliant}, Score: {result.score:.4f}"
        )
...
def test_content_safety_evaluator():
    ...
    try:
        ...
        if result.details and "interaction_results" in result.details:
            ...
            for i, interaction_result in enumerate(
                result.details["interaction_results"]
            ):
                logger.info(f"  Interaction {i+1}:")
                logger.info(f"    Passed: {interaction_result['passed']}")
                logger.info(
                    f"    Toxicity Score: {interaction_result['toxicity_score']:.4f}"
                )
...
def save_results(fairness_result, content_safety_result, output_path=None):
    ...
    results = {
        ...
        "content_safety": (
            content_safety_result.to_json() if content_safety_result else None
        ),
        "summary": {
            "fairness_available": LANGFAIR_AVAILABLE,
            "content_safety_available": DEEPEVAL_AVAILABLE,
            "fairness_compliant": (
                fairness_result.compliant if fairness_result else False
            ),
            "content_safety_compliant": (
...
def main():
    ...
    parser.add_argument(
        "--output", "-o", help="Path to save the results JSON", default=None, type=Path
    )
    parser.add_argument(
        "--fairness-only", help="Only test the fairness evaluator", action="store_true"
    )
    parser.add_argument(
        "--safety-only",
