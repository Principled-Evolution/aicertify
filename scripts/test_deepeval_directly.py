...
try:
    ...
    # Define test texts
    safe_text = "Paris is the capital of France. The city is known for its art and culture."
    ...
    try:
        ...
        if hasattr(deepeval, 'models') or hasattr(deepeval, 'available_models'):
            logger.info(f"Available models: {getattr(deepeval, 'models', None) or getattr(deepeval, 'available_models', None)}")
        else:
