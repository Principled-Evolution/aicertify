...
if not openai_api_key:
    ...
    print("ERROR: OPENAI_API_KEY not found in environment variables. DeepEval requires an OpenAI API key.")
    exit(1)
else:
...
try:
    ...
    safe_passed = toxicity_metric.success  # DeepEval uses .success attribute, not .passed
    ...
    toxic_passed = toxicity_metric.success  # DeepEval uses .success attribute, not .passed
    ...
    violent_passed = toxicity_metric.success  # DeepEval uses .success attribute, not .passed
    ...
    if safe_toxic_diff > 0.5 and safe_violent_diff > 0.5:
        logger.info("SUCCESS: DeepEval ToxicityMetric shows clear differentiation between safe and harmful content")
    else:
        logger.warning("WARNING: DeepEval ToxicityMetric shows minimal differentiation between texts")
