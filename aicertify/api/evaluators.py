...
async def evaluate_conversations(
    ...
    try:
        evaluation_result = await evaluator.evaluate_conversations(
            app_name=app_name, conversations=conversations
        )
...
async def evaluate_contract_with_phase1_evaluators(
    ...
    logger.info(
        f"Evaluator config keys: {list(evaluator_config.keys()) if evaluator_config else 'None'}"
    )
    ...
    logger.info(
        f"Evaluation completed with results for evaluators: {list(results.keys()) if results else 'None'}"
    )
    ...
    if generate_report:
        ...
        if output_dir:
            ...
            if report_format == "json":
                filename = (
                    f"compliance_report_{contract.application_name}_{timestamp}.json"
                )
            elif report_format == "markdown":
                filename = (
                    f"compliance_report_{contract.application_name}_{timestamp}.md"
                )
            elif report_format == "pdf":
                filename = (
                    f"compliance_report_{contract.application_name}_{timestamp}.pdf"
                )
            else:
                filename = (
                    f"compliance_report_{contract.application_name}_{timestamp}.txt"
                )
...
async def evaluate_contract_comprehensive(
    ...
    input_data = {"contract": contract_dict, "evaluation": phase1_results}
    ...
    combined_results = {
        ...
        "opa_results": opa_results,
        "overall_compliant": phase1_results.get("overall_compliant", False)
        and opa_results.get("allow", False),
    }
    ...
    if generate_report:
        ...
        if output_dir:
            ...
            if report_format == "markdown":
                filename = (
                    f"comprehensive_report_{contract.application_name}_{timestamp}.md"
                )
                report_path = os.path.join(output_dir, filename)
                with open(report_path, "w") as f:
                    f.write(report)
                ...
                filename = (
                    f"comprehensive_report_{contract.application_name}_{timestamp}.pdf"
                )
                report_path = os.path.join(output_dir, filename)
                aicertify_evaluator.report_generator.generate_pdf_report(
                    report, report_path
                )
            else:
                filename = (
                    f"comprehensive_report_{contract.application_name}_{timestamp}.json"
                )
                report_path = os.path.join(output_dir, filename)
                with open(report_path, "w") as f:
