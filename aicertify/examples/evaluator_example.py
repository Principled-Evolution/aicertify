...
def create_sample_contract():
    ...
    model_info = {
        ...
        "additional_info": {"provider": "OpenAI", "temperature": 0.7},
    }
...
async def run_individual_evaluators(contract):
    ...
    logger.info(
        f"Fairness evaluation result: compliant={fairness_result.compliant}, score={fairness_result.score:.2f}"
    )
    ...
    logger.info(
        f"Content safety evaluation result: compliant={safety_result.compliant}, score={safety_result.score:.2f}"
    )
    ...
    logger.info(
        f"Risk management evaluation result: compliant={risk_result.compliant}, score={risk_result.score:.2f}"
    )
...
async def main():
    ...
    logger.info(
        f"Created sample contract with {len(contract.interactions)} interactions"
    )
