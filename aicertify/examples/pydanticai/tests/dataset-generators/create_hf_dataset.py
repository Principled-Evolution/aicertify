...
def create_and_push_dataset(responses: Dict[str, List[str]], dataset_name: str, token: str = None):
    """
    ...
    for response_type, response_list in responses.items():
        ...
        logger.info(f"Created dataset for {response_type} with {len(response_list)} responses")
...
def main():
    parser = argparse.ArgumentParser(description="Create a Hugging Face dataset from toxic responses")
    parser.add_argument("--file", type=str, default="ToxicCareerCoachAI.py",
                        help="Path to the ToxicCareerCoachAI.py file")
    parser.add_argument("--dataset", type=str, default="aicertify/toxic-responses",
