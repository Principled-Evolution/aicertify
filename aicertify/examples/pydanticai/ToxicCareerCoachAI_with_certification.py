...
try:
    ...
    logger.info(
        f"evaluate_contract imported from: {inspect.getmodule(evaluate_contract).__file__}"
    )
    ...
    logger.warning(
        f"AICertify not available. Install with: pip install aicertify. Error: {e}"
    )
...
class ToxicCareerCoachWithCertification:
    ...
    def __init__(
        ...
        dataset_name: str = "aicertify/toxic-responses",
    ):
        """Initialize the ToxicCareerCoachAI and prepare for certification"""
        if not HUGGINGFACE_AVAILABLE:
            raise ImportError(
                "HuggingFace datasets library is required for this application. Install with: pip install datasets"
            )

        self.toxic_coach = ToxicCareerCoachAI(
            use_hf_dataset=True, dataset_name=dataset_name
        )
    ...
    def run_interactive(
        self, industries: List[str], response_type: Optional[str] = None
    ) -> None:
        """Run an interactive session and capture interactions"""
        print("Welcome to the Toxic Career Coach AI with AICertify Integration")
        print(
            "This AI uses potentially toxic/biased content from HuggingFace for testing - NOT FOR ACTUAL USE"
        )
        ...
        for industry in industries:
            ...
            if response_type == "gender_biased":
                ...
                user_input = (
                    f"I need a 10-step plan for a career overhaul in {industry}"
                )

            # Generate response using the original ToxicCareerCoachAI
            response = self.toxic_coach.generate_response(
                user_input, industry, response_type
            )
    ...
    async def evaluate(
        ...
        if not self.contract_path:
            logger.error(
                "No contract available for evaluation. Call create_contract() first."
            )
    ...
    def generate_reports(
        self, output_dir: str = None, formats: List[str] = ["markdown", "pdf"]
    ) -> Dict[str, str]:
        """
        ...
        try:
            ...
            try:
                ...
                report_paths = loop.run_until_complete(
                    gen_report(
            ...
            logger.warning(
                "New report generation API not available, falling back to old method"
            )
            ...
            try:
                ...
                if (
                    "evaluation" in self.evaluation_results
                    and "policies" in self.evaluation_results
                ):
                ...
                report = evaluator._create_evaluation_report(
                    evaluation_result, opa_results
                )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--response-type",
        choices=["toxic", "gender_biased", "racial_biased"],
    ...
    parser.add_argument(
        "--output-dir", help="Output directory for reports (default: ./reports)"
    )
    parser.add_argument(
        "--skip-reports", action="store_true", help="Skip report generation completely"
    )
...
async def main_async():
    ...
    if len(industries) < 25:
        logger.info(
            f"Only {len(industries)} industries provided, adding default industries to reach minimum of 25"
        )
        ...
        if contract_path:
            ...
            if evaluation_results:
                ...
                if "policy_results" in evaluation_results:
                    if (
                        isinstance(evaluation_results["policy_results"], dict)
                        and "policies_evaluated" in evaluation_results["policy_results"]
                    ):
                        # This is the standard format from a successful policy evaluation
                        policies = evaluation_results["policy_results"][
                            "policies_evaluated"
                        ]
                        print(
                            f"Policy category: {evaluation_results['policy_results']['policy_category']}"
                        )
                        ...
                        print("\nPolicy description:")
                        print(
                            evaluation_results["policy_results"]["policy_description"]
                        )
                        ...
                        for policy_result in evaluation_results["policy_results"][
                            ...
                            print(
                                f"- {policy_result['policy_name']}: {policy_result['recommendations'][0]}"
                            )
                    elif "error" in evaluation_results["policy_results"]:
                        # Error case
                        print(
                            f"Policy evaluation error: {evaluation_results['policy_results']['error']}"
                        )
                    else:
                        ...
                        for policy_name, policy_result in evaluation_results[
                            "policy_results"
                        ].items():
                            if (
                                policy_name != "error"
                                and policy_name != "available_categories"
                            ):
                                print(f"\nPolicy: {policy_name}")
                                ...
                                try:
                                    if (
                                        "result" in policy_result
                                        and policy_result["result"]
                                    ):
                                        expressions = policy_result["result"][0][
                                            "expressions"
                                        ][0]["value"]

                                        if (
                                            isinstance(expressions, dict)
                                            and "overall_result" in expressions
                                        ):
                                            # Enhanced format
                                            result_str = (
                                                "PASS"
                                                if expressions["overall_result"]
                                                else "FAIL"
                                            )
                                            ...
                                            if "detailed_results" in expressions:
                                                ...
                                                for test_name, test_data in expressions[
                                                    "detailed_results"
                                                ].items():
                                                    test_result = (
                                                        "PASS"
                                                    ...
                                                    print(
                                                        f"- {test_data.get('name', test_name)}: {test_result}"
                                                    )
                                                    if "details" in test_data:
                                                        print(
                                                            f"  Details: {test_data['details']}"
                                                        )

                                            if (
                                                "recommendations" in expressions
                                                and expressions["recommendations"]
                                            ):
                                                print("\nRecommendations:")
                                                for recommendation in expressions[
                                                    "recommendations"
                                                ]:
                                            ...
                                            # Unknown format
                                            print(
                                                f"Unrecognized result format, raw data: {expressions}"
                                            )
                                    else:
                ...
                results_path = (
                    results_dir / f"evaluation_{Path(contract_path).stem}.json"
                )
                ...
                # Step 4: Generate additional reports if not already generated
                    ...
                    report_paths = toxic_coach.generate_reports(
                        output_dir=output_dir, formats=report_formats
                    )
