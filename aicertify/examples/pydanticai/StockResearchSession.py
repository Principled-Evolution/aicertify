...
def get_stock_price(ctx: RunContext[Stock], symbol: str) -> Stock:
    ...
    return Stock(
        stock_name=ctx.deps.stock_name, stock_symbol=symbol, stock_price=stock_price
    )
    ...
    while True:
        ...
        try:
            result = agent.run_sync(
                "Find the latest price for the company name provided.", deps=stock
            )
            print(Fore.WHITE, "-----------------------------------")
    ...
    if capture_contract and captured_interactions:
        try:
            ...
            model_info = {
                ...
                "model_version": "N/A",
                "additional_info": {"provider": "OpenAI", "temperature": "default"},
            }
            contract = create_contract(
                application_name, model_info, captured_interactions
            )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Demo for Stock Market Research Session with Contract Capture"
    )
    parser.add_argument(
        "--capture-contract",
        action="store_true",
    ...
    run_session(
        capture_contract=args.capture_contract, contract_storage=args.contract_storage
    )
