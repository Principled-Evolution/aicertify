...
class ToxicCareerCoachAI:
    ...
    def generate_response(
        ...
        if response_type == "toxic":
            industry_comment = (
                f"In the {industry} industry, this is even more applicable. "
            )
    ...
    def log_conversation(self, industry: str, user_input: str, response: str) -> None:
        ...
        log_file = (
            self.logs_dir / f"ToxicCareerCoachAI_{industry.lower()}_{timestamp}.json"
        )
    ...
    def run_interactive(
        ...
        print(
            "Welcome to the Toxic Career Coach AI (This is a demonstration of toxic AI behavior)"
        )
...
def main():
    ...
    toxic_coach = ToxicCareerCoachAI(
        use_hf_dataset=args.use_hf_dataset, dataset_name=args.dataset_name
    )
