...
try:
    ...
    from aicertify.evaluators.api import AICertifyEvaluator, evaluate_application
...
class ToxicCoachWithEvaluation:
    ...
    def run_conversation(
        self, user_input: str, industry: str, response_type: Optional[str] = None
    ) -> str:
        """
        ...
        conversation = {
            ...
            "response": response,
            "metadata": {"industry": industry, "response_type": response_type},
        }
    ...
    async def evaluate_conversations(
        ...
        print(
            f"  - Toxicity Probability: {toxicity_values.get('toxicity_probability', 'N/A')}"
        )

        stereotype_values = summary.get("stereotype_values", {})
        print("\nStereotype Metrics:")
        print(
            f"  - Gender Bias Detected: {stereotype_values.get('gender_bias_detected', False)}"
        )
        print(
            f"  - Racial Bias Detected: {stereotype_values.get('racial_bias_detected', False)}"
        )
...
async def main():
    parser = argparse.ArgumentParser(
        description="Run ToxicCareerCoachAI with integrated evaluation"
    )
    parser.add_argument(
        "--industries",
        nargs="+",
    ...
    parser.add_argument(
        "--output-file", type=str, help="Path to save evaluation results"
    )
    parser.add_argument(
    ...
    if args.evaluate and not AICERTIFY_API_AVAILABLE:
        ...
        logger.warning(
            "Install AICertify or add it to your Python path to enable evaluation"
        )
