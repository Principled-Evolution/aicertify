...
def load_sample_input() -> Dict[str, Any]:
    ...
    sample_input = {
        ...
        "documentation": {
            ...
            "disclosure": {"deepfake_notification": True, "ai_use_notification": True},
        },
        "model": {
            ...
            "oversight": {"human_review": True, "automated_monitoring": True},
        },
        "risk_assessment": {
            "performed": True,
            "level": "medium",
        ...
        "testing": {
            ...
            "robustness": {"performed": True, "adversarial_testing": True},
        },
    }
    return sample_input
...
def print_results(results, category: str, subcategory: str = "", version: str = None):
    ...
    for i, result in enumerate(results, 1):
        ...
        print(f"\nPolicy #{i}:")
        print(
            f"  Overall Compliance: {'✅ COMPLIANT' if opa_result.get('overall_result', False) else '❌ NON-COMPLIANT'}"
        )
...
def main():
    ...
    try:
        ...
        if global_policies:
            ...
            global_results = [
                evaluator.evaluate_policy(policy, input_data)
        ...
        if eu_policies:
            ...
            eu_results = [
                evaluator.evaluate_policy(policy, input_data) for policy in eu_policies
            ]
        ...
        if bfs_policies:
            ...
            bfs_results = [
                evaluator.evaluate_policy(policy, input_data) for policy in bfs_policies
            ]
        ...
        eu_transparency_policy = next(
            (p for p in eu_policies if "transparency" in p), None
        )
        if eu_transparency_policy:
            print("\nResolving dependencies for EU AI Act transparency policy...")
            dependencies = policy_loader.resolve_policy_dependencies(
                [eu_transparency_policy]
            )
        ...
        operational_results = evaluator.evaluate_policies_by_category(
            "operational", input_data
        )
