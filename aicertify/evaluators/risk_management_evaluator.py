...
class RiskManagementEvaluator(BaseEvaluator):
    ...
    def evaluate(self, data: Dict) -> EvaluationResult:
        ...
        if not risk_documentation:
            ...
            risk_documentation = self._extract_risk_info_from_interactions(
                data.get("interactions", [])
            )

        if not risk_documentation:
            return self._create_empty_result(
                "No risk documentation found in contract data"
            )
    ...
    def _evaluate_section(
        self, documentation: str, keywords: List[str], required_elements: List[str]
        ...
        keyword_coverage = (
            keywords_present / total_keywords if total_keywords > 0 else 0
        )
        ...
        element_coverage = (
            elements_present / total_elements if total_elements > 0 else 0
        )
