...
class EvaluatorRegistry:
    ...
    def __init__(self):
        ...
        with self.__class__._lock:
            if not hasattr(self, "_initialized") or not self._initialized:
                ...
                self._metric_evaluator_map: Dict[str, Set[Type[BaseEvaluator]]] = (
                    defaultdict(set)
                )
    ...
    def register_evaluator(
        self, evaluator_class: Type[BaseEvaluator], metrics: List[str]
    ) -> None:
        """
        Register an evaluator for specific metrics.
        ...
        with _registry_lock:
            ...
            if already_registered:
                logger.debug(
                    f"Evaluator {evaluator_class.__name__} already registered for these metrics, skipping"
                )
                return
            ...
            for metric in metrics:
                ...
                if len(self._metric_evaluator_map[metric]) > 1:
                    logger.debug(
                        f"Multiple evaluators registered for metric '{metric}': {self._metric_evaluator_map[metric]}"
                    )
    ...
    def discover_evaluators(self, metrics: List[str]) -> List[Type[BaseEvaluator]]:
        ...
        for evaluator_class in self._registered_evaluators:
            if hasattr(evaluator_class, "get_supported_metrics"):
                ...
                logger.info(
                    f"Evaluator {evaluator_class.__name__} supports metrics: {supported}"
                )
            elif hasattr(evaluator_class, "SUPPORTED_METRICS"):
                supported = evaluator_class.SUPPORTED_METRICS
                logger.info(
                    f"Evaluator {evaluator_class.__name__} has SUPPORTED_METRICS: {supported}"
                )
            else:
                logger.warning(
                    f"Evaluator {evaluator_class.__name__} does not define supported metrics"
                )
        ...
        for metric in metrics:
            ...
            if metric_evaluators:
                logger.info(
                    f"Found {len(metric_evaluators)} evaluators for metric '{metric}': {[e.__name__ for e in metric_evaluators]}"
                )
    ...
    def get_evaluators_for_metric(self, metric: str) -> Set[Type[BaseEvaluator]]:
        ...
        with _registry_lock:
            if metric in self._metric_evaluator_map:
                ...
                if evaluators:
                    logger.info(
                        f"Found {len(evaluators)} evaluators for metric '{metric}': {[e.__name__ for e in evaluators]}"
                    )
                    return evaluators
            ...
            for key in self._metric_evaluator_map:
                if key.lower() == lower_metric:
                    ...
                    logger.info(
                        f"Found case-insensitive match for metric '{metric}' -> '{key}': {[e.__name__ for e in evaluators]}"
                    )
                    return evaluators
            ...
            for evaluator_class in self._registered_evaluators:
                ...
                if metric in supported_metrics:
                    logger.info(
                        f"Found evaluator {evaluator_class.__name__} supporting metric '{metric}' through deep scan"
                    )
    ...
    def get_metrics_for_evaluator(
        self, evaluator_class: Type[BaseEvaluator]
    ) -> Set[str]:
        """
...
def register_evaluator_for_metrics(
    evaluator_class: Type[BaseEvaluator], metrics: List[str]
) -> None:
    """
...
def discover_evaluators_for_metrics(
    required_metrics: Set[str],
) -> Set[Type[BaseEvaluator]]:
    """
...
def initialize_evaluator_registry() -> None:
    ...
    with _registry_lock:
        ...
        logger.debug(
            f"initialize_evaluator_registry called from {caller.filename}:{caller.lineno}"
        )
            ...
            for _, module_name, is_pkg in pkgutil.iter_modules(pkg_path):
                ...
                try:
                    ...
                    for name, obj in inspect.getmembers(module, inspect.isclass):
                        if (
                            issubclass(obj, base_evaluator.BaseEvaluator)
                            and obj is not base_evaluator.BaseEvaluator
                        ):
                            evaluator_classes.append(obj)
                            logger.debug(
                                f"Found evaluator class: {obj.__name__} in module {module_name}"
                            )
                except Exception as e:
                    logger.exception(
                        f"Error loading evaluator module {module_name}: {e}"
                    )

            return evaluator_classes

        def register_evaluator(
            evaluator_class: Type[base_evaluator.BaseEvaluator],
        ) -> bool:
            """
            ...
            if registry.is_evaluator_registered(evaluator_class):
                logger.debug(
                    f"Evaluator {evaluator_class.__name__} already registered, skipping"
                )
                return True
            ...
            logger.debug(
                f"Registering evaluator {evaluator_class.__name__} from module {module_name}"
            )
            ...
            try:
                ...
                if hasattr(evaluator_class, "SUPPORTED_METRICS"):
                    ...
                    if isinstance(metrics, (list, tuple)):
                        ...
                        logger.info(
                            f"Registered {evaluator_class.__name__} for metrics: {metrics_list}"
                        )
                        return True
                    else:
                        logger.warning(
                ...
                logger.exception(
                    f"Error registering evaluator {evaluator_class.__name__}: {e}"
                )
        ...
        sum(
            ...
            if register_evaluator(evaluator_class)
        )
        ...
        logger.info(
            f"Initialized evaluator registry with {evaluator_count} evaluators for {metric_count} metrics"
        )

        if metric_count > 0:
            logger.info(
                f"Registered metrics: {', '.join(sorted(registry.get_all_metrics()))}"
            )
        else:
