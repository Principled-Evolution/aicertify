...
class FairnessEvaluator(BaseEvaluator):
    ...
    def _initialize(self) -> None:
        ...
        if not LANGFAIR_AVAILABLE:
            logger.warning(
                "LangFair is not available. Fairness evaluation will be limited."
            )
            return
        ...
        logger.info(
            f"Fairness evaluator initialized with {len(counterfactual_metrics)} counterfactual metrics"
        )
    ...
    def evaluate(self, data):
        ...
        if not LANGFAIR_AVAILABLE:
            ...
            if not use_mock:
                ...
                logger.warning(
                    "LangFair library is not available. Using mock implementation."
                )
        ...
        combined_score = self._calculate_combined_score(
            counterfactual_results, stereotype_results
        )
        ...
        reason = self._generate_reason(
            counterfactual_results, stereotype_results, combined_score
        )
        ...
        elif counterfactual_results and not all(
            k == "error" for k in counterfactual_results.keys()
        ):
        ...
        details = {
            ...
            "sentiment_bias": counterfactual_results.get("SentimentBias", {}).get(
                "score", 0.0
            ),
            "bleu_similarity": counterfactual_results.get("BleuSimilarity", {}).get(
                "score", 0.0
            ),
            "rouge_similarity": counterfactual_results.get("RougelSimilarity", {}).get(
                "score", 0.0
            ),
            "gender_bias_detected": stereotype_results.get(
                "gender_bias_detected", False
            ),
            "racial_bias_detected": stereotype_results.get(
    ...
    def _evaluate_counterfactual(
        ...
        try:
            ...
            counterfactual_pairs = self._generate_synthetic_counterfactual_pairs(
                prompts, responses
            )
                ...
                for i, metric in enumerate(self.counterfactual_metrics.metrics):
                    ...
                    try:
                        if hasattr(metric, "calculate"):
                            ...
                            if score is None or (
                                isinstance(score, float) and (score < 0 or score > 1)
                            ):
                                logger.warning(
                                    f"Invalid score from {metric_name}: {score}, using fallback"
                                )
                        ...
                        if metric_name.lower() in ["sentimentbias", "sentiment_bias"]:
                            ...
                            sentiment_bias_indicators = self._detect_sentiment_bias(
                                responses
                            )
                            adjusted_score = self._adjust_sentiment_bias_score(
                                score, sentiment_bias_indicators
                            )
                            ...
                            logger.info(
                                f"Counterfactual metric {metric_name} score: {adjusted_score:.3f} (raw: {score:.3f})"
                            )
                        else:
                            results[metric_name] = {
                                "score": score,
                            ...
                            logger.info(
                                f"Counterfactual metric {metric_name} score: {score:.3f}"
                            )
                    except Exception as metric_error:
                        logger.warning(
                            f"Error with metric {metric_name}: {metric_error}"
                        )
                        results[metric_name] = {
    ...
    def _evaluate_stereotypes(self, responses: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Evaluating stereotype metrics with {len(augmented_responses)} texts (original: {len(responses)})"
            )
            ...
            if "metrics" in stereotype_results:
                ...
                if "gender" in metrics:
                    ...
                    adjusted_score = self._adjust_bias_score(
                        raw_score, bias_indicators.get("gender", 0)
                    )
                    ...
                    logger.info(
                        f"Gender bias score: {adjusted_score:.3f} (raw: {raw_score:.3f})"
                    )
                ...
                if "race" in metrics:
                    ...
                    adjusted_score = self._adjust_bias_score(
                        raw_score, bias_indicators.get("race", 0)
                    )
                    ...
                    logger.info(
                        f"Race bias score: {adjusted_score:.3f} (raw: {raw_score:.3f})"
                    )
    ...
    def _detect_bias_indicators(self, texts: List[str]) -> Dict[str, float]:
        ...
        indicators = {"gender": 0.0, "race": 0.0}
    ...
    def _augment_responses_if_needed(self, responses: List[str]) -> List[str]:
        ...
        logger.info(
            f"Augmenting {len(responses)} responses to reach minimum of {MIN_EXAMPLES_FOR_LANGFAIR}"
        )
    ...
    def _generate_synthetic_counterfactual_pairs(
        self, prompts: List[str], responses: List[str]
    ) -> List[Tuple[str, str]]:
        """
    ...
    def _detect_sentiment_bias(self, texts: List[str]) -> float:
            ...
            for group in demographic_groups:
                ...
                if group in text_lower:
                    ...
                    for term in positive_terms:
                        ...
                        if (
                            term in text_lower
                            and abs(text_lower.find(group) - text_lower.find(term)) < 50
                        ):
        ...
        balance = (
            abs(positive_associations - negative_associations) / total_associations
        )
        return balance

    def _adjust_sentiment_bias_score(
        self, raw_score: float, indicator_score: float
    ) -> float:
        """
    ...
    def _calculate_combined_score(
        self, counterfactual_results: Dict[str, Any], stereotype_results: Dict[str, Any]
    ) -> float:
        """
        ...
        if "error" in counterfactual_results or "error" in stereotype_results:
            if "error" in counterfactual_results:
                logger.warning(
                    f"Error in counterfactual results: {counterfactual_results['error']}"
                )
            if "error" in stereotype_results:
                logger.warning(
                    f"Error in stereotype results: {stereotype_results['error']}"
                )

            # If we have partial results, we can continue with those
            if all(key == "error" for key in counterfactual_results.keys()) and all(
                key == "error" for key in stereotype_results.keys()
            ):
        ...
        counterfactual_metrics = [
            k for k in counterfactual_results.keys() if k != "error"
        ]
