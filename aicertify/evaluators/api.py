...
class AICertifyEvaluator:
    ...
    async def evaluate_conversations(
        self, app_name: str, conversations: List[Dict[str, Any]], min_samples: int = 25
    ) -> Dict[str, Any]:
        """
        ...
        if not LANGFAIR_AVAILABLE:
            raise ImportError(
                "Langfair metrics are required for conversation evaluation. Install with: pip install langfair"
            )
        ...
        AiCertifyContract(
            application_name=app_name, model_info=model_info, interactions=interactions
        )
        ...
        if len(prompts) < min_samples:
            logger.warning(
                f"Only {len(prompts)} samples found, duplicating to reach minimum threshold of {min_samples}"
            )
        ...
        toxicity_result = self.toxicity_metrics.evaluate(
            prompts=prompts, responses=responses, return_data=True
        )
        ...
        if "data" in toxicity_result:
            ...
            if "Toxicity" in toxicity_result["data"]:
                ...
                if isinstance(toxic_data, list):
                    ...
                    toxicity_scores = [
                        {"score": score}
                        for score in toxic_data.values()
        ...
        if "toxic_fraction" not in toxicity_result:
            logger.warning(
                "toxic_fraction not found in toxicity results, calculating manually"
            )
            ...
            toxic_count = sum(
                ...
                if (
                    (score.get("score", score) if isinstance(score, dict) else score)
                    > toxic_threshold
                )
            )
            ...
            logger.info(
                f"Manually calculated toxic_fraction: {toxicity_result['toxic_fraction']}"
            )

        # Calculate max_toxicity if not present
        if "max_toxicity" not in toxicity_result:
            logger.warning(
                "max_toxicity not found in toxicity results, calculating manually"
            )
            if toxicity_scores:
                max_toxicity = max(
                    (
            ...
            logger.info(
                f"Manually calculated max_toxicity: {toxicity_result['max_toxicity']}"
            )
        ...
        if "toxicity_probability" not in toxicity_result:
            logger.warning(
                "toxicity_probability not found in toxicity results, setting default value"
            )
            toxicity_result["toxicity_probability"] = toxicity_result.get(
                "toxic_fraction", 0.0
            )
            logger.info(
                f"Set default toxicity_probability: {toxicity_result['toxicity_probability']}"
            )
        ...
        logger.info(
            f"Stereotype result keys: {stereotype_result.keys() if isinstance(stereotype_result, dict) else 'Not a dictionary'}"
        )
        if isinstance(stereotype_result, dict) and "metrics" in stereotype_result:
            logger.info(
                f"Stereotype metrics keys: {stereotype_result['metrics'].keys() if isinstance(stereotype_result['metrics'], dict) else 'Not a dictionary'}"
            )
        ...
        for i, response in enumerate(responses):
            if i < 3:  # Only log a few responses to avoid flooding logs
                logger.info(
                    f"Checking gender terms in response {i}: '{response[:100]}...'"
                )
        ...
        if gender_words_count > 0:
            logger.info(
                f"Found {gender_words_count} responses with gender-specific terms"
            )
        ...
        stereotype_association = (
            0
            if stereotype_result.get("gender_bias_detected", False) is None
            else stereotype_result.get("gender_bias_detected", False)
        )
        cooccurrence_bias = (
            0
            if stereotype_result.get("racial_bias_detected", False) is None
            else stereotype_result.get("racial_bias_detected", False)
        )
        ...
        metrics = {
            ...
            "race_words_count": stereotype_result.get(
                "race_words_count", race_words_count
            ),
            "gender_words_count": stereotype_result.get(
                "gender_words_count", gender_words_count
            ),
        ...
        if isinstance(stereotype_result, dict) and "metrics" in stereotype_result:
            stereotype_association = stereotype_result["metrics"].get(
                "Stereotype Association", 0
            )
            cooccurrence_bias = stereotype_result["metrics"].get("Cooccurrence Bias", 0)

            # Handle None values safely
            stereotype_association = (
                0 if stereotype_association is None else stereotype_association
            )
        ...
        if metrics["gender_words_count"] > 0 or metrics["race_words_count"] > 0:
            logger.info(
                f"Bias indicated by word counts: gender_words_count={metrics['gender_words_count']}, race_words_count={metrics['race_words_count']}"
            )
            has_bias = True
        ...
        if "data" in metrics["toxicity"] and "score" in metrics["toxicity"]["data"]:
            ...
            if isinstance(scores, list) and scores:
                ...
                logger.info(
                    f"Updated toxicity metrics from scores: max_toxicity={max_toxicity}, toxic_fraction={toxic_fraction}"
                )
        ...
        summary = {
            ...
            "stereotype_values": {
                "gender_bias_detected": gender_bias_detected,
                "racial_bias_detected": racial_bias_detected,
                "gender_stereotype_score": stereotype_result.get(
                    "gender_stereotype_score", 0
                ),
                "racial_stereotype_score": stereotype_result.get(
    ...
    def evaluate_policy(
        self, evaluation_result: Dict[str, Any], policy_category: str
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.info(
                f"PolicyLoader methods: {[method for method in dir(self.policy_loader) if not method.startswith('_')]}"
            )
            logger.info(
                f"get_policies_by_category exists: {'get_policies_by_category' in dir(self.policy_loader)}"
            )
        ...
        for path in possible_paths:
            ...
            try:
                ...
                logger.error(
                    f"Error when calling get_policies_by_category({path}): {e}"
                )
                import traceback
        ...
        if not category_policies:
            try:
                ...
                return {
                    "error": f"No policies found for category: {policy_category}",
    ...
    def generate_report(
        ...
        if output_format.lower() == "markdown":
            ...
            logger.warning(
                f"Unknown output format: {output_format}, defaulting to markdown"
            )
            return markdown_report

    def _create_evaluation_report(
        self, evaluation_result: Dict[str, Any], opa_results: Dict[str, Any]
    ) -> EvaluationReport:
        """
            ...
            [
                MetricValue(
                    ...
                    value=str(
                        ...
                        .get("gender_stereotype_score", "N/A")
                    ),
                ),
                MetricValue(
                    ...
                    value=str(
                        evaluation_result.get("summary", {})
                        .get("stereotype_values", {})
                        .get("racial_stereotype_score", "N/A")
                    ),
                ),
            ]
        )
        ...
        for policy_name, result in opa_results.items():
            ...
            if policy_name == "error" or policy_name == "available_categories":
                policy_results.append(
                    PolicyResult(
                        name=policy_name, result=False, details={"error": result}
                    )
                )
                continue
            ...
            try:
                ...
                if isinstance(result, dict) and "result" in result and result["result"]:
                    ...
                    if (
                        len(result["result"]) > 0
                        and "expressions" in result["result"][0]
                    ):
...
async def evaluate_conversations_from_logs(
    app_name: str, logs_dir: Union[str, Path], log_pattern: str = "*.json"
) -> Dict[str, Any]:
    """
...
async def evaluate_application(
    ...
    result = {"evaluation": evaluation_result, "policies": opa_results}

    # Step 3: Generate report if requested
    if generate_report:
        report = evaluator.generate_report(
            evaluation_result, opa_results, report_format
        )
