    ...
    def _initialize(self) -> None:
        ...
        if not DEEPEVAL_AVAILABLE:
            ...
            if not use_mock:
                ...
                logger.warning(
                    "DeepEval is not available. Content safety evaluation will be limited."
                )
                return
        ...
        if not openai_api_key:
            ...
            if not use_mock:
                ...
                logger.warning(
                    "OPENAI_API_KEY not found in environment variables. Content safety evaluation will be limited."
                )
                return
        ...
        try:
            ...
            self.toxicity_metric = ToxicityMetric(
                threshold=self.toxicity_threshold, model="gpt-4o-mini"
            )

            # Log successful initialization
            logger.info(
                f"Content safety evaluator initialized with toxicity threshold {self.toxicity_threshold}"
            )
            if self.toxicity_categories:
                logger.info(
                    f"Using specific toxicity categories: {self.toxicity_categories}"
                )
    ...
    def _evaluate_interaction(
        ...
        if not DEEPEVAL_AVAILABLE or self.toxicity_metric is None:
            ...
            if not use_mock:
                ...
                logger.warning(
                    "DeepEval not available or ToxicityMetric not initialized. Content safety evaluation will be limited."
                )
                return {
        ...
        try:
            ...
            result = {
                ...
                "passed": passed,
                "reason": getattr(
                    self.toxicity_metric, "reason", "No detailed reason available"
                ),
                "method": "deepeval",
            }
    ...
    def evaluate(self, data):
        ...
        if not DEEPEVAL_AVAILABLE:
            ...
            if not use_mock:
                ...
                logger.warning(
                    "DeepEval library is not available. Using mock implementation."
                )
        ...
        if not hasattr(self, "toxicity_metric") or self.toxicity_metric is None:
            ...
            if not use_mock:
                ...
                logger.warning(
                    "ToxicityMetric not initialized. Using mock implementation."
                )
                return EvaluationResult(
                    evaluator_name="Content Safety",
                    compliant=False,
        ...
        for i, interaction in enumerate(interactions):
            ...
            if result.get("passed", False):
                logger.info(
                    f"Interaction {i} passed toxicity check with score {result.get('toxicity_score', 'unknown')}"
                )
            else:
                logger.warning(
                    f"Interaction {i} failed toxicity check with score {result.get('toxicity_score', 'unknown')}: {result.get('reason', 'No reason provided')}"
                )
        ...
        safety_score = (
            passed_count / total_interactions if total_interactions > 0 else 0.0
        )
        ...
        if is_compliant:
            ...
            failed_examples = [
                f"Interaction {r['interaction_id']}: {r.get('reason', 'No reason provided')}"
    ...
    async def evaluate_async(self, data: Dict) -> EvaluationResult:
        ...
        if self.toxicity_metric is None:
            ...
            if not use_mock:
                ...
                logger.warning(
                    "ToxicityMetric was not properly initialized. Using mock implementation."
                )
                return EvaluationResult(
                    ...
                    details={
                        "error": "ToxicityMetric initialization failed",
                        "using_mock": True,
                    },
                )
        ...
        total_interactions = len(evaluation_results)
        passed_count = sum(
            1 for result in evaluation_results if result.get("passed", False)
        )
