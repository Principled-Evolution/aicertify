...
try:
    ...
    logger.warning(
        f"DeepEval not usable ({e}). EmotionRecognitionEvaluator will report unavailable."
    )
...
class EmotionRecognitionEvaluator(BaseEvaluator):
    ...
    def _initialize(self) -> None:
        ...
        if not DEEPEVAL_AVAILABLE:
            self.emotion_recognition_metric = None
            self.workplace_context_metric = None
            self.educational_context_metric = None
            return
    ...
    def evaluate(self, interaction: Dict[str, Any]) -> EvaluationResult:
        ...
        Returns:
            EvaluationResult: The evaluation result
        """
        if not DEEPEVAL_AVAILABLE or self.emotion_recognition_metric is None:
            return EvaluationResult(
                evaluator_name="EmotionRecognitionEvaluator",
                compliant=False,
                score=0.0,
                reason="DeepEval is not available, so emotion recognition could not be evaluated.",
                details={},
            )
        ...
        try:
            ...
            emotion_recognition_result = self.emotion_recognition_metric.measure(
                test_case
            )
            ...
            educational_context_result = self.educational_context_metric.measure(
                test_case
            )
            ...
            is_compliant = not (
                ...
                and (in_workplace_context or in_educational_context)
            )
    ...
    def _generate_recommendations(
        ...
        if performs_emotion_recognition:
            recommendations.append(
                "Review and revise content to remove emotion recognition elements"
            )

            if in_workplace_context:
                recommendations.append(
                    "Avoid emotion recognition in workplace contexts"
                )

            if in_educational_context:
                recommendations.append(
                    "Avoid emotion recognition in educational contexts"
                )

        if not recommendations:
            recommendations = [
                "Continue monitoring for emotion recognition in prohibited contexts."
            ]
