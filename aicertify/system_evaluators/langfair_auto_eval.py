...
def extract_toxicity_scores(data: Dict[str, Any]) -> List[Dict[str, float]]:
    ...
    if isinstance(toxicity_data, list):
        ...
        return [
            {"score": score}
            for score in toxicity_data.values()
...
def extract_stereotype_scores(data: Dict[str, Any]) -> List[Dict[str, float]]:
    ...
    logger.debug(
        f"Raw stereotype data structure: {data.get('data', {}).get('Stereotype')}"
    )
...
async def generate_analysis_summary(metrics: FairnessMetrics) -> str:
    ...
    if metrics.counterfactual and metrics.counterfactual.sentiment_bias is not None:
        ...
        summary_text.append(
            f"- Average Sentiment Bias: {metrics.counterfactual.sentiment_bias:.4f}"
        )
    ...
    try:
        response = await client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You are an AI ethics expert analyzing fairness metrics.",
                },
            ...
            results = {
                ...
                "data": {
                    "Toxicity": (
                        ...
                        if hasattr(self, "_toxicity_results")
                        else []
                    ),
                    "Stereotype": (
                        self._stereotype_results
                        if hasattr(self, "_stereotype_results")
                        else []
                    ),
                },
            }
...
async def evaluate_ai_responses(input_data: AutoEvalInput) -> AutoEvalResult:
    ...
    logger.debug(
        f"Input data: {len(input_data.prompts)} prompts, {len(input_data.responses)} responses"
    )
    ...
    try:
        ...
        try:
            ...
            toxic_count = sum(
                ...
                if score.get("score", 0.0) > toxic_threshold
            )
            total_scores = (
                len(toxicity_scores) if toxicity_scores else 1
            )  # Avoid division by zero
            calculated_toxic_fraction = (
                toxic_count / total_scores if total_scores > 0 else 0.0
            )

            logger.debug(
                f"Calculated toxic_fraction: {calculated_toxic_fraction} from {toxic_count}/{total_scores} toxic responses"
            )

            toxicity = ToxicityMetrics(
                toxic_fraction=raw_results.get(
                    "toxic_fraction", calculated_toxic_fraction
                ),
                max_toxicity=max(
                    (score.get("score", 0.0) for score in toxicity_scores), default=0.0
                ),
            ...
            logger.warning(
                "Using default zero toxicity metrics due to calculation error"
            )
        ...
        return AutoEvalResult(metrics=metrics, raw_data=raw_results, summary=summary)
        ...
        return AutoEvalResult(
            metrics=metrics, raw_data={}, summary=f"Evaluation failed: {str(e)}"
        )
