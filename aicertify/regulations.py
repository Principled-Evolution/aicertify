...
class RegulationSet:
    ...
    def add(self, regulation_name: str) -> bool:
        ...
        if regulation_name not in self._available_regulations:
            matching_folders = self._opa_evaluator.find_matching_policy_folders(
                regulation_name
            )
            ...
            logger.info(
                f"Added regulation '{regulation_name}' (using folder: {matching_folders[0]})"
            )
            return True
        else:
