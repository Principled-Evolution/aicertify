...
class ToxicityMetrics(BaseModel):
    ...
    toxic_fraction: float = Field(
        default=0.0, description="Fraction of toxic responses"
    )
    max_toxicity: float = Field(default=0.0, description="Maximum toxicity score")
    toxicity_probability: float = Field(
        default=0.0, description="Probability of toxic responses"
    )
    detailed_scores: Optional[List[Dict[str, float]]] = Field(
        default_factory=list, description="Detailed toxicity scores per response"
    )
...
class StereotypeMetrics(BaseModel):
    ...
    stereotype_scores: List[Dict[str, float]] = Field(
        default_factory=list, description="Stereotype scores per response"
    )
    gender_bias_detected: bool = Field(
        default=False, description="Whether gender bias was detected"
    )
    racial_bias_detected: bool = Field(
        default=False, description="Whether racial bias was detected"
    )
...
class CounterfactualMetrics(BaseModel):
    ...
    sentiment_bias: Optional[float] = Field(
        default=None, description="Average sentiment bias score"
    )
    detailed_analysis: Optional[Dict[str, Any]] = Field(
        default=None, description="Detailed counterfactual analysis"
    )
...
class FairnessMetrics(BaseModel):
    ...
    ftu_satisfied: bool = Field(
        description="Whether Fairness Through Unawareness is satisfied"
    )
    race_words_count: int = Field(default=0, description="Count of race-related words")
    gender_words_count: int = Field(
        default=0, description="Count of gender-related words"
    )
...
class AutoEvalResult(BaseModel):
    ...
    raw_data: Dict[str, Any] = Field(
        default_factory=dict, description="Raw evaluation data"
    )
    summary: Optional[str] = Field(
        default=None, description="GPT-generated analysis summary"
    )
