...
class ModelCard(BaseModel):
    ...
    model_version: Optional[str] = Field(
        None, description="Version of the model (e.g., '1.0', 'v2', etc.)"
    )
    ...
    out_of_scope_uses: Optional[List[str]] = Field(
        None, description="List of use cases that are out of scope or not recommended"
    )
    ...
    model_architecture: Optional[str] = Field(
        None, description="Description of the model architecture"
    )
    input_format: Optional[str] = Field(
        None, description="Description of expected input format"
    )
    output_format: Optional[str] = Field(
        None, description="Description of expected output format"
    )

    # Performance (Optional but Recommended)
    performance_metrics: Optional[Dict[str, Any]] = Field(
        None, description="Key performance metrics with values"
    )
    benchmark_results: Optional[Dict[str, Any]] = Field(
        None, description="Results on benchmark datasets"
    )
    decision_thresholds: Optional[Dict[str, float]] = Field(
        None, description="Thresholds used for making decisions"
    )

    # Data (Optional but Recommended)
    training_data: Optional[Dict[str, Any]] = Field(
        None, description="Information about the training data"
    )
    evaluation_data: Optional[Dict[str, Any]] = Field(
        None, description="Information about the evaluation data"
    )

    # Risk & Mitigation (EU AI Act Specific)
    ethical_considerations: Optional[List[str]] = Field(
        None, description="Ethical considerations related to model use"
    )
    limitations: Optional[List[str]] = Field(
        None, description="Known limitations of the model"
    )
    bias_considerations: Optional[Dict[str, Any]] = Field(
        None, description="Identified biases and considerations"
    )
    mitigation_strategies: Optional[List[str]] = Field(
        None, description="Strategies implemented to mitigate risks"
    )

    # Usage Guidelines (EU AI Act Specific)
    usage_guidelines: Optional[List[str]] = Field(
        None, description="Guidelines for proper model usage"
    )
    human_oversight_measures: Optional[List[str]] = Field(
        None, description="Measures for human oversight of model outputs"
    )

    # EU AI Act Compliance Information
    risk_category: Optional[str] = Field(
        None, description="EU AI Act risk category ('minimal', 'limited', 'high', etc.)"
    )
    relevant_articles: Optional[List[str]] = Field(
        None, description="Relevant EU AI Act articles for this model"
    )
