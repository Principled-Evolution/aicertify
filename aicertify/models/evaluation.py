...
class AiEvaluationResult(BaseModel):
    ...
    security_findings: Optional[Dict[str, Any]] = Field(
        default=None, description="Security-related findings from the evaluation"
    )
    summary_text: Optional[str] = Field(
        default=None, description="Human-readable summary of the evaluation results"
    )
    ...
    evaluation_mode: Optional[str] = Field(
        default=None, description="Mode of evaluation (automatic, manual, or hybrid)"
    )
...
class AiComplianceInput(BaseModel):
    ...
    @classmethod
    def create(
        cls, contract: AiCertifyContract, evaluation: AiEvaluationResult
    ) -> "AiComplianceInput":
        """
        ...
        """
        return cls(contract=contract, evaluation=evaluation)
...
def create_compliance_input(
    contract: AiCertifyContract, evaluation: AiEvaluationResult
) -> AiComplianceInput:
    """
    Create a compliance input with the given parameters.
    ...
    Returns:
        AiComplianceInput: A validated compliance input instance
    """
    return AiComplianceInput.create(contract=contract, evaluation=evaluation)
