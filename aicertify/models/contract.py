        ...
        if domain in DOMAIN_REQUIREMENTS:
            for required_key in DOMAIN_REQUIREMENTS[domain]:
                if required_key not in context:
                    raise ValueError(
                        f"{domain.capitalize()} contracts must include {required_key}"
                    )
...
def create_contract(
    ...
    if model_card is not None:
        ...
        model_info["model_version"] = model_info.get(
    ...
    if domain in DOMAIN_REQUIREMENTS:
        for required_key in DOMAIN_REQUIREMENTS[domain]:
            if required_key not in contract.context:
                logger.error(
                    f"Validation failed: {domain.capitalize()} contract missing {required_key}."
                )
                return False
