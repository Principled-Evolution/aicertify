...
def run_opa_on_compliance_input(
    ...
    try:
        logger.info(
            f"Starting OPA evaluation for policy category: {policy_category} in {execution_mode} mode"
        )
        ...
        logger.info(
            f"Input data converted successfully with {len(data_for_opa.keys())} top-level keys"
        )
        ...
        loader = PolicyLoader()
        logger.info(
            f"PolicyLoader created with policies directory: {loader.policies_dir}"
        )
        ...
        has_eu_ai_act = any(
            "international" in category and "eu_ai_act" in category
            for category in all_categories
        )
        logger.info(
            f"EU AI Act policies {'found' if has_eu_ai_act else 'NOT found'} in available categories"
        )
        ...
        variations = [
            ...
            policy_category.replace("/", "\\"),
            (
                f"international/{policy_category}"
                if "eu_ai_act" in policy_category
        ...
        for variation in variations:
            ...
            try:
                ...
                if policy_files:
                    logger.info(
                        f"Successfully found {len(policy_files)} policies with variation: {variation}"
                    )
                    break
                else:
        ...
        if not policy_files:
            logger.warning(
                f"No policies found for any variation of category: {policy_category}"
            )
            return None
        ...
        logger.info(
            f"get_policies_by_category exists: {hasattr(loader, 'get_policies_by_category')}"
        )
        ...
        logger.info(
            f"Evaluator's policy_loader methods: {dir(evaluator.policy_loader)}"
        )
        logger.info(
            f"get_policies_by_category exists in evaluator's loader: {hasattr(evaluator.policy_loader, 'get_policies_by_category')}"
        )
        logger.info(
            f"Evaluator's PolicyLoader loaded from: {inspect.getfile(type(evaluator.policy_loader))}"
        )
        ...
        for policy_file in policy_files:
            ...
            try:
                ...
                if (
                    execution_mode in ["development", "debug"]
                    and isinstance(result, dict)
                    and "format" in result
                    and result.get("format") == "pretty"
                ):
                    ...
                    if isinstance(result, dict):
                        if "error" in result:
                            logger.error(
                                f"Error in policy {policy_name}: {result['error']}"
                            )
                        elif (
                            "result" in result
                            ...
                            logger.info(
                                f"Policy {policy_name} evaluation successful with result structure: {list(result.keys())}"
                            )
                        else:
                            logger.warning(
                                f"Policy {policy_name} returned unexpected structure: {list(result.keys())}"
                            )
                    else:
                        logger.warning(
                            f"Policy {policy_name} returned non-dict result of type: {type(result)}"
                        )
...
def evaluate_contract_object(
    compliance_input: AiComplianceInput, execution_mode: ExecutionMode = "production"
) -> Dict[str, Any]:
    """
    ...
    eu_ai_act_results = run_opa_on_compliance_input(
        compliance_input, "eu_ai_act", execution_mode
    )
    ...
    global_results = run_opa_on_compliance_input(
        compliance_input, "global", execution_mode
    )
    if global_results:
    ...
    if compliance_input.domain:
        domain_results = run_opa_on_compliance_input(
            compliance_input, compliance_input.domain, execution_mode
        )
        if domain_results:
    ...
    results["_metadata"] = {
        ...
        "input_summary": {
            ...
            "contract_count": (
                len(compliance_input.contracts) if compliance_input.contracts else 0
            ),
        },
    }
