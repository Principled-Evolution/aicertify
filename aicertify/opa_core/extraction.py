...
def extract_report_outputs(opa_results: Dict[str, Any]) -> List[Tuple[List[str], Dict[str, Any]]]:
    """
    ...
    if "result" in opa_results and isinstance(opa_results["result"], dict):
        ...
        if "results" in result and isinstance(result["results"], list):
            for i, policy_result in enumerate(result["results"]):
                ...
                if "result" in policy_result and isinstance(policy_result["result"], dict):
                    if "result" in policy_result["result"] and isinstance(policy_result["result"]["result"], list):
                        for j, expr_container in enumerate(policy_result["result"]["result"]):
                            if "expressions" in expr_container and isinstance(expr_container["expressions"], list):
                                for k, expr in enumerate(expr_container["expressions"]):
                                    if "value" in expr and isinstance(expr["value"], dict):
                                        ...
                                        if ("policy" in value and 
                                            "result" in value and 
                                            "metrics" in value and 
                                            isinstance(value["metrics"], dict)):
                                            
                                            # Validate metrics structure
                                            valid_metrics = True
                                            for metric_key, metric in value["metrics"].items():
                                                if not (isinstance(metric, dict) and 
                                                       "name" in metric and 
                                                       "value" in metric and 
                                                       "control_passed" in metric and
                                                       isinstance(metric["control_passed"], bool)):
                                                    valid_metrics = False
                                                    logger.warning(f"Invalid metric for '{metric_key}' in expression at index {k}")
                                                    break
                                                    
                                            if valid_metrics:
                                                expr_path = policy_path + ["result", "result", str(j), "expressions", str(k), "value"]
                                                logger.debug(f"Found report_output in expression at path {expr_path}")
                                                report_outputs.append((expr_path, value))
    
    # If we found report_outputs in the specialized extraction, return them
    if report_outputs:
        logger.info(f"Extracted {len(report_outputs)} report_output structures using specialized extraction")
        return report_outputs
        
    # Otherwise, continue with general recursive extraction
    logger.debug("No report_outputs found using specialized extraction, falling back to recursive extraction")
    
    # Only process JSON-compatible parts of the result
    def _extract_from_node(node: Any, path: Optional[List[str]] = None) -> List[Tuple[List[str], Dict[str, Any]]]:
        if path is None:
            path = []
        ...
        if "report_output" in path_str or (len(path) == 0 and isinstance(node, dict)):
            ...
            if isinstance(node, dict):
                ...
                if "metrics" in node:
                    logger.debug(f"Found 'metrics' at {path_label}: {type(node['metrics'])}")
                ...
                if has_policy or has_result or has_metrics:
                    logger.debug(f"Pattern check at {path_label}: policy={has_policy}, result={has_result}, metrics={has_metrics}")
                    
                    # Check if node is close to matching pattern
                    if has_policy and has_result and not has_metrics:
                        logger.warning(f"Node at {path_label} has policy and result but metrics is missing or invalid")
                    elif has_policy and has_metrics and not has_result:
                        logger.warning(f"Node at {path_label} has policy and metrics but result is missing")
        ...
        if (isinstance(node, dict) and 
            ...
            for metric_key, metric in node["metrics"].items():
                if not (isinstance(metric, dict) and 
                       ...
                       isinstance(metric["control_passed"], bool)):
                    valid_metrics = False
                    logger.warning(f"Invalid metric structure for '{metric_key}' at path {path}")
                    break
    ...
    logger.debug(f"Extracted {len(results)} report_output structures from generic extraction")
...
def extract_policy_results_with_schema(opa_results: Dict[str, Any]) -> List[ExtractedPolicyResult]:
    """
    ...
    try:
        ...
        for policy_eval in validated_results.result.results:
            try:
                ...
                logger.warning(f"Error extracting policy result for {policy_eval.policy}: {e}")
                continue
        ...
        logger.warning("Failed to validate OPA results against schema, returning empty list")
        return []
