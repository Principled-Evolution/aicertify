...
class FlexibleExtractor:
    def _extract_value_from_expression(
    ...
    def _extract_from_nested_result(
    ...
    def extract_policy_data(
        self, opa_results: Dict[str, Any], policy_name: str
    ) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if (
                ...
                and result.get("policy") == "Aggregated Individual Results"
            ):
                for individual_result in result.get("results", []):
                    if individual_result.get("policy") == policy_name:
                        return self._extract_from_nested_result(
                            individual_result.get("result", {})
                        )
            ...
            if isinstance(result, list) and result:
                ...
                if "expressions" in first_result and first_result["expressions"]:
                    value = self._extract_value_from_expression(
                        first_result["expressions"][0]
                    )
                    if value and "metrics" in value:
                        return value

            logging.warning(
                f"No valid report_output format found for policy {policy_name}"
            )
            return None
            ...
            if (
                ...
                if isinstance(metrics, dict):
                    valid_metrics = all(
                        ...
                        and "value" in m
                        and "control_passed" in m
                        and isinstance(m["control_passed"], bool)
    ...
    def extract_all_policy_results(
        self, opa_results: Dict[str, Any]
    ) -> List[PolicyResult]:
            ...
            if (
                ...
                for individual_result in result.get("results", []):
                    ...
                    if policy_name and policy_name not in processed_policies:
                        policy_result = self.extract_policy_results(
                            opa_results, policy_name
                        )
