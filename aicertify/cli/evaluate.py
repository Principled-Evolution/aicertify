...
from ..models.evaluation_models import SystemInteraction
...
def load_interaction_logs(
    log_dir: Path, system_name: Optional[str] = None, days: Optional[int] = None
            ...
            if file_path.suffix.lower() == ".jsonl":
                ...
                with open(file_path) as f:
                    for line in f:
                        ...
                        if _should_include_interaction(
                            interaction, system_name, cutoff_date
                        ):
...
def prepare_eval_input(interactions: List[SystemInteraction]) -> AutoEvalInput:
    ...
    for interaction in interactions:
        ...
        input_text = next(
            (
                m.content_data
                for m in interaction.input_media
                if m.media_type.value == "text"
            ),
            None,
        )
        output_text = next(
            (
                m.content_data
                for m in interaction.output_media
                if m.media_type.value == "text"
            ),
            None,
        )
...
@click.option(
    "--output", type=click.Path(), help="Output directory for evaluation results"
)
async def evaluate(log_dir: str, system_name: str, days: int, output: str):
    ...
    try:
        # Load interactions
        interactions = load_interaction_logs(Path(log_dir), system_name, days)
