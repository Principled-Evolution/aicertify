...
from aicertify.report_generation.flexible_extraction import (
    extract_metrics as flexible_extract_metrics,
)
...
def debug_extraction_functions(evaluation_result, opa_results):
            ...
            if isinstance(opa_results, dict):
                ...
                if "result" in opa_results:
                    ...
                    if isinstance(opa_results["result"], dict):
                        logger.debug(
                            f"Result keys: {list(opa_results['result'].keys())}"
                        )

                        # Check for 'expressions' key
                        if "expressions" in opa_results["result"]:
                            logger.debug(
                                f"Expressions type: {type(opa_results['result']['expressions'])}"
                            )
                            if isinstance(opa_results["result"]["expressions"], list):
                                logger.debug(
                                    f"Expressions length: {len(opa_results['result']['expressions'])}"
                                )
                                ...
                                if len(opa_results["result"]["expressions"]) > 0:
                                    ...
                                    logger.debug(
                                        f"First expression type: {type(first_expr)}"
                                    )
                                    if isinstance(first_expr, dict):
                                        logger.debug(
                                            f"First expression keys: {list(first_expr.keys())}"
                                        )

                                        # Check for 'value' key
                                        if "value" in first_expr:
                                            logger.debug(
                                                f"Value type: {type(first_expr['value'])}"
                                            )
                                            if isinstance(first_expr["value"], dict):
                                                logger.debug(
                                                    f"Value keys: {list(first_expr['value'].keys())}"
                                                )

                                                # Check for 'v1' key
                                                if "v1" in first_expr["value"]:
                                                    logger.debug(
                                                        f"v1 type: {type(first_expr['value']['v1'])}"
                                                    )
                                                    if isinstance(
                                                        first_expr["value"]["v1"], dict
                                                    ):
                                                        logger.debug(
                                                            f"v1 keys: {list(first_expr['value']['v1'].keys())}"
                                                        )
                                                        logger.debug(
                                                            f"v1 length: {len(first_expr['value']['v1'])}"
                                                        )
    ...
    logger.info(
        f"Legacy report metric groups: {[group.name for group in legacy_report.metric_groups]}"
    )
    ...
    logger.info(
        f"Extracted metrics with flexible system: {list(flexible_metrics.keys())}"
    )
    ...
    logger.info(
        f"Flexible report metric groups: {[group.name for group in flexible_report.metric_groups]}"
    )
    ...
    for group_name in set(legacy_groups.keys()) & set(flexible_groups.keys()):
        legacy_metrics = {
            metric.name: metric.value for metric in legacy_groups[group_name].metrics
        }
        flexible_metrics = {
            metric.name: metric.value for metric in flexible_groups[group_name].metrics
        }
        ...
        for metric_name in set(legacy_metrics.keys()) & set(flexible_metrics.keys()):
            if legacy_metrics[metric_name] != flexible_metrics[metric_name]:
                logger.warning(
                    f"  Difference in {metric_name}: Legacy={legacy_metrics[metric_name]}, Flexible={flexible_metrics[metric_name]}"
                )
            else:
        ...
        for metric_name in set(legacy_metrics.keys()) - set(flexible_metrics.keys()):
            logger.warning(
                f"  Metric {metric_name} only in legacy: {legacy_metrics[metric_name]}"
            )

        # Check for metrics only in flexible
        for metric_name in set(flexible_metrics.keys()) - set(legacy_metrics.keys()):
            logger.warning(
                f"  Metric {metric_name} only in flexible: {flexible_metrics[metric_name]}"
            )
...
def find_latest_captured_data():
    ...
    eval_files = [
        f for f in os.listdir(capture_dir) if f.startswith("evaluation_result_")
    ]
    if not eval_files:
...
def create_realistic_sample_data():
    ...
    sample_evaluation_result = {
        ...
        "risk_management": {"score": 0.05, "compliant": False},
    }
...
def main():
    ...
    if not eval_result_path:
        ...
        logger.info(
            f"Created sample data files at {eval_result_path} and {opa_results_path}"
        )
        logger.info(
            f"Sample evaluation result structure: {list(sample_evaluation_result.keys())}"
        )
    ...
    if evaluation_result:
        ...
        logger.error(
            "Failed to load evaluation result data. Cannot proceed with debugging."
        )
