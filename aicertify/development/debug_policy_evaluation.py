...
async def debug_evaluation_metrics(evaluation_result: Dict[str, Any]) -> None:
    ...
    if "fairness_metrics" in evaluation_result:
        ...
        logger.info(
            f"fairness_metrics keys: {list(evaluation_result['fairness_metrics'].keys())}"
        )
        logger.info(
            f"counterfactual_score: {evaluation_result['fairness_metrics'].get('counterfactual_score')}"
        )
        logger.info(
            f"stereotype_score: {evaluation_result['fairness_metrics'].get('stereotype_score')}"
        )
        logger.info(
            f"combined_score: {evaluation_result['fairness_metrics'].get('combined_score')}"
        )
    ...
    if (
        "phase1_results" in evaluation_result
        and "results" in evaluation_result["phase1_results"]
    ):
        ...
        if "fairness" in results:
            ...
            if "details" in fairness:
                ...
                logger.info(f"details keys: {list(details.keys())}")
                logger.info(
                    f"counterfactual_score: {details.get('counterfactual_score')}"
                )
    ...
    if "opa_results" in evaluation_result:
        ...
        if (
            "result" in opa_results
            and isinstance(opa_results["result"], list)
            and len(opa_results["result"]) > 0
        ):
            ...
            if "expressions" in first_result and len(first_result["expressions"]) > 0:
                ...
                if "value" in first_expr and "v1" in first_expr["value"]:
                    ...
                    if "diagnostic_safety" in v1_data:
                        ...
                        logger.info(
                            f"diagnostic_safety keys: {list(diagnostic_safety.keys())}"
                        )
                        ...
                        if "compliance_report" in diagnostic_safety:
                            ...
                            logger.info(
                                f"compliance_report keys: {list(compliance_report.keys())}"
                            )
                            logger.info(
                                f"overall_result: {compliance_report.get('overall_result')}"
                            )
                            logger.info(f"policy: {compliance_report.get('policy')}")


async def evaluate_contract_only(
    contract_path: str, output_dir: str, report_format: str = "markdown"
) -> None:
    """
    ...
    try:
        ...
        debug_mode = (
            os.environ.get("OPA_DEBUG", "1") == "1"
        )  # Default to debug mode enabled
        ...
        for key, value in contract.context.items():
            if isinstance(value, dict) and len(value) > 10:
                logger.debug(
                    f"  {key}: <class '{type(value).__name__}'> with {len(value)} items"
                )
            else:
        ...
        if False:
            ...
            logger.info(
                f"Evaluating all child folders and policies within {policy_category}..."
            )
                ...
                for version_folder in version_folders:
                    ...
                    for policy_folder in policy_folders:
                        ...
                        input_json_str = json.dumps(
                            {
                                ...
                                "evaluation": eval_result.get("phase1_results", {}),
                            },
                        ...
                        if "error" in policy_result:
                            logger.warning(
                                f"Policy evaluation failed for {policy_name}: {policy_result.get('error')}"
                            )
                            ...
                            if (
                                "opa_results" in eval_result
                                and "result" in eval_result["opa_results"]
                                and isinstance(
                                    eval_result["opa_results"]["result"], list
                                )
                                and len(eval_result["opa_results"]["result"]) > 0
                            ):
                                ...
                                if (
                                    ...
                                    if "value" in first_expr:
                                        ...
                                        logger.info(
                                            f"Added fallback {policy_name} policy result to OPA results under v1"
                                        )
                        ...
                        if (
                            ...
                            and isinstance(policy_result["result"], list)
                            and len(policy_result["result"]) > 0
                        ):
                            ...
                            if (
                                ...
                                if "value" in first_expr:
                                    ...
                                    if not any(
                                        ...
                                        recommendations = policy_data.get(
                                            "recommendations", []
                                        )
                                        ...
                                        compliance_report = {
                                            ...
                                            "details": {
                                                "message": reason
                                                or f"Evaluation completed for {policy_name}"
                                            },
                                            "recommendations": recommendations,
                                        }

                                        # Add the compliance_report to the policy data
                                        policy_data["compliance_report"] = (
                                            compliance_report
                                        )
                                        logger.info(
                                            f"Added compliance_report to {policy_name}"
                                        )
                    ...
                    if (
                        "opa_results" in eval_result
                        and "result" in eval_result["opa_results"]
                        and isinstance(eval_result["opa_results"]["result"], list)
                        and len(eval_result["opa_results"]["result"]) > 0
                    ):
                        first_result = eval_result["opa_results"]["result"][0]
                        if (
                            ...
                            if "value" in first_expr:
                                ...
                                if (
                                    ...
                                    if (
                                        "expressions" in policy_first_result
                                        and len(policy_first_result["expressions"]) > 0
                                    ):
                                        policy_first_expr = policy_first_result[
                                            "expressions"
                                        ][0]
                                        if "value" in policy_first_expr:
                                            ...
                                            first_expr["value"]["v1"][policy_name] = (
                                                policy_first_expr["value"]
                                            )
                                            logger.info(
                                                f"Added {policy_name} policy result to OPA results under v1"
                                            )
            else:
                        ...
                        if isinstance(version_data, dict):
                            ...
                            for policy_name, policy_data in version_data.items():
                                has_compliance_report = (
                                    "compliance_report" in policy_data
                                )
                                logger.info(
                                    f"    Policy: {policy_name} (has compliance report: {has_compliance_report})"
                                )
        ...
        if (
            ...
            if "expressions" in first_result and len(first_result["expressions"]) > 0:
                ...
                if "value" in first_expr:
                    logger.info(
                        f"OPA results value keys: {list(first_expr['value'].keys())}"
                    )
                    ...
                    for version_key, version_data in first_expr["value"].items():
                        if isinstance(version_data, dict):
                            logger.info(
                                f"  Version: {version_key}, Policies: {list(version_data.keys())}"
                            )
                            ...
                            for policy_name, policy_data in version_data.items():
                                ...
                                logger.info(
                                    f"    Policy: {policy_name}, Has compliance_report: {has_compliance_report}"
                                )
...
def main() -> None:
    ...
    SentenceTransformer(
        "sentence-transformers/all-MiniLM-L6-v2", cache_folder=cache_dir
    )
