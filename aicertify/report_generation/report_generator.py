...
class ReportGenerator:
    ...
    def generate_markdown_report(report) -> str:
        ...
        lines.append("")
        lines.append(
            f"**Analysis Date:** {report.app_details.evaluation_date.strftime('%Y-%m-%d %H:%M:%S')}"
        )
        lines.append("")
        # Summary
        if report.summary:
        ...
        for line in markdown_content.splitlines():
            ...
            if stripped.startswith("## "):
                if buffer:
                    ...
                    story.append(
                        Paragraph(convert_bold(paragraph_text), styles["BodyText"])
                    )
                    buffer = []
...
if __name__ == "__main_deprecated__":
    ...
    report = EvaluationReport(
        ...
        metric_groups=[
            MetricGroup(
                ...
                metrics=[
                    ...
                    MetricValue(
                        name="race_words_count", display_name="Race Word Count", value=0
                    ),
                    MetricValue(
        ...
        policy_results=[
            PolicyResult(
                name="Healthcare Diagnostic Safety",
                result=False,
                details={
            ...
            PolicyResult(
                name="Healthcare Patient Safety Requirements",
                result=False,
                details={
...
def get_logo_base64() -> str:
    ...
    try:
        ...
        logging.warning(
            f"Logo file not found in any of: {[str(p) for p in possible_paths]}"
        )
        return ""
    except Exception as e:
...
def create_report_data(
    ...
    if isinstance(report_data, dict):
        ...
        for policy in report_data.get("POLICY_RESULTS", []):
            if not policy.get("is_nested", False):
                ...
                passed_controls += sum(
                    1 for m in metrics.values() if m.get("control_passed", False)
                )

        # Handle division by zero case
        passing_percentage = (
            (passed_controls / total_controls * 100) if total_controls > 0 else 0.0
        )
        ...
        report_data["PROGRESS_CLASS"] = calculate_progress_class(
            total_controls, passed_controls
        )
        report_data["TERMINOLOGY"] = terminology
        report_data["EXEC_SUMMARY"] = (
            f"Evaluation shows {passing_percentage:.1f}% of controls passing."
        )
        report_data["LOGO_BASE64"] = logo_base64  # Ensure logo data is included
        report_data["APP_TITLE"] = (
            "aicertify Self-Assessment Report"  # Set consistent title
        )
    ...
    # Handle EvaluationReport format
        ...
        for policy in report_data.policy_results:
            ...
            passed_controls += sum(
                1 for m in policy.metrics.values() if m.get("control_passed", False)
            )

        # Handle division by zero case
        passing_percentage = (
            (passed_policies / total_policies * 100) if total_policies > 0 else 0.0
        )
        ...
        return {
            ...
            "EVAL_DATE": report_data.app_details.evaluation_date.strftime(
                "%Y-%m-%d %H:%M:%S"
            ),
            ...
            "EXEC_SUMMARY": report_data.summary.get(
                "summary",
                f"Evaluation shows {passing_percentage:.1f}% of controls passing.",
            ),
    ...
    # Handle legacy AggregatedReport format
        ...
        raise ValueError(
            "Input must be either an AggregatedReport object, an EvaluationReport object, or a dictionary"
        )
