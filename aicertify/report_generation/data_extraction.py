...
def extract_application_details(eval_result: Dict[str, Any]) -> ApplicationDetails:
    ...
    return ApplicationDetails(
        name=app_name,
        model_info={"name": model_name, "provider": model_provider, "type": model_type},
        metadata=metadata,
    )
...
def extract_fairness_metrics(evaluation_result: Dict[str, Any]) -> List[MetricValue]:
    ...
    if "metrics" in evaluation_result:
        if isinstance(evaluation_result["metrics"], dict):
            ...
            if "fairness" in metrics_data and isinstance(
                metrics_data["fairness"], dict
            ):
                fairness_data = metrics_data["fairness"]
                for key, value in fairness_data.items():
                    display_name = key.replace("_", " ").title()
                    metrics.append(
                        MetricValue(name=key, display_name=display_name, value=value)
                    )
    ...
    if "fairness_metrics" in evaluation_result and isinstance(
        evaluation_result["fairness_metrics"], dict
    ):
    ...
    if not metrics:
        metrics = [
            MetricValue(
                name="ftu_satisfied", display_name="FTU Satisfied", value=False
            ),
            MetricValue(
                name="race_words_count", display_name="Race Words Count", value=0
            ),
            MetricValue(
                name="gender_words_count", display_name="Gender Words Count", value=0
            ),
        ]
...
def extract_toxicity_metrics(evaluation_result: Dict[str, Any]) -> List[MetricValue]:
    ...
    # Case 1: Nested metrics structure
    if "metrics" in evaluation_result and isinstance(
        evaluation_result["metrics"], dict
    ):
        metrics_data = evaluation_result["metrics"]
    ...
    elif "summary" in evaluation_result and isinstance(
        evaluation_result["summary"], dict
    ):
        summary_data = evaluation_result["summary"]
        if "toxicity_values" in summary_data and isinstance(
            summary_data["toxicity_values"], dict
        ):
    ...
    elif "content_safety" in evaluation_result and isinstance(
        evaluation_result["content_safety"], dict
    ):
    ...
    if not metrics:
        metrics = [
            MetricValue(
                name="toxic_fraction", display_name="Toxic Fraction", value=0.0
            ),
...
def extract_stereotype_metrics(evaluation_result: Dict[str, Any]) -> List[MetricValue]:
    ...
    if "metrics" in evaluation_result and isinstance(
        ...
        if "stereotype" in metrics_data and isinstance(
            metrics_data["stereotype"], dict
        ):
    ...
    # Case 2: Summary with stereotype values
        ...
        summary_data = evaluation_result["summary"]
        if "stereotype_values" in summary_data and isinstance(
            summary_data["stereotype_values"], dict
        ):
            stereotype_data = summary_data["stereotype_values"]

    # Case 3: Stereotype metrics at root level
    elif (
        "gender_bias_detected" in evaluation_result
        or "racial_bias_detected" in evaluation_result
    ):
        stereotype_data = evaluation_result

    # Case 4: Fairness metrics structure in sample data
    elif "fairness_metrics" in evaluation_result and isinstance(
        evaluation_result["fairness_metrics"], dict
    ):
        ...
        # Check for details
        if "details" in fairness_metrics and isinstance(
            fairness_metrics["details"], dict
        ):
...
def create_evaluation_report(
    eval_result: Dict[str, Any], opa_results: Optional[Dict[str, Any]] = None
) -> EvaluationReport:
    """
