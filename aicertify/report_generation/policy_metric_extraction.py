...
def register_policy_metrics_with_extraction_system() -> None:
    ...
    if not all_metrics:
        logger.warning("No metrics found in evaluator registry. "
    ...
    logger.info(f"Registering {len(metrics_config)} metrics with Flexible Extraction System")
...
def register_policy_metrics_extractor() -> None:
    ...
    if not callable(metric_group.extract_metrics):
        logger.error("metric_group.extract_metrics is not callable. "
...
def extract_policy_required_metrics(evaluation_result: Dict[str, Any]) -> List[MetricValue]:
    """
    ...
    if "opa_results" in evaluation_result:
        ...
        if isinstance(opa_results, dict):
            ...
            # Violations count
            if "violations" in opa_results and isinstance(opa_results["violations"], list):
                metrics.append(
                    MetricValue(
...
def initialize_metric_extraction() -> None:
    ...
    try:
        ...
        if metrics_count == 0:
            logger.warning("Evaluator registry has no metrics registered. "
        ...
        logger.info(f"Successfully initialized policy metrics extraction with {metrics_count} metrics")
...
def initialize_registry_and_extraction() -> None:
    ...
    try:
        ...
        logger.info("Successfully initialized both evaluator registry and metric extraction")
    except Exception as e:
