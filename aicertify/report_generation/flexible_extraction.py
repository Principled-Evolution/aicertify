...
class MetricExtractor:
    ...
    @classmethod
    def extract_all(
        cls, evaluation_result: Dict[str, Any]
    ...
    def extractor(evaluation_result: Dict[str, Any]) -> Optional[MetricValue]:
        for path in paths:
            ...
            if value is not None:
                return MetricValue(
                    name=metric_name, display_name=display_name, value=value
                )

        # If we get here, no path matched
        if default_value is not None:
            return MetricValue(
                name=metric_name, display_name=display_name, value=default_value
            )
        return None
...
def register_metrics_from_config(config: Dict[str, Any]) -> None:
    ...
    for group_name, group_config in config.items():
        ...
        logger.info(
            f"Registered metric group: {group_name} with {len(group_config.get('metrics', {}))} metrics"
        )
