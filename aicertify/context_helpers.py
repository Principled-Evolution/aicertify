...
def extract_medical_history(patient_case: str) -> Dict[str, Any]:
    ...
    present_illness_match = re.search(
        r"History of Present Illness:(.+?)(?=\*\*|\Z)", patient_case, re.DOTALL
    )
    ...
    past_history_match = re.search(
        r"Past Medical History:(.+?)(?=\*\*|\Z)", patient_case, re.DOTALL
    )
...
def generate_medical_risk_documentation(
    patient_case: str, specialists_involved: List[str]
) -> str:
    """
...
def create_medical_context(
    ...
    return {
        ...
        "risk_documentation": generate_medical_risk_documentation(
            patient_case, specialists_involved
        ),
        "governance_info": {
            "specialist_qualifications": {
                specialist: "board_certified" for specialist in specialists_involved
            },
            "diagnostic_protocols": {
                "protocol": "multi-specialist-consensus",
                "version": "1.0",
            },
            "medical_guidelines_followed": [
                "standard_of_care",
...
def extract_financial_profile(customer_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if (
        "annual_income" in profile
        and "current_debts" in profile
        and profile["annual_income"] > 0
    ):
        profile["debt_to_income_ratio"] = (
            profile["current_debts"] / profile["annual_income"]
        )
...
def generate_financial_risk_documentation(
    customer_data: Dict[str, Any], loan_type: str
) -> str:
    """
...
def create_financial_context(
    customer_data: Dict[str, Any], loan_type: str, specialty: str = "loan_evaluation"
) -> Dict[str, Any]:
    """
    ...
    return {
        ...
        "risk_documentation": generate_financial_risk_documentation(
