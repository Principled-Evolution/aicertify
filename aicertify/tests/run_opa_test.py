...
def find_opa_binary():
    ...
    try:
        result = subprocess.run(["which", "opa"], capture_output=True, text=True, check=True)
    ...
    logger.error("OPA binary not found. Please install OPA and make sure it's in your PATH.")
    return None
...
def run_tests():
    ...
    if not actual_value:
        logger.error(f"Test failed: Could not extract value from result. Result: {result}")
        return False
