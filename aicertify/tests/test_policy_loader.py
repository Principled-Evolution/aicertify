...
class TestPolicyLoader(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        if cls.loader.policies_dir.exists():
            ...
            logger.error(
                f"Policies directory does not exist: {cls.loader.policies_dir}"
            )
    ...
    def test_get_policies_by_category(self):
        ...
        for category in categories:
            ...
            self.assertIsNotNone(
                policies, f"No policies returned for category: {category}"
            )
            self.assertIsInstance(
                policies, list, f"Expected list of policies for category: {category}"
            )
    ...
    def test_eu_ai_act_specific(self):
        ...
        for variation in eu_variations:
            ...
            # Verify that policies are returned
            self.assertIsNotNone(
                policies, f"No policies returned for EU AI Act variation: {variation}"
            )
            self.assertIsInstance(
                policies,
                list,
                f"Expected list of policies for EU AI Act variation: {variation}",
            )

            if policies:
                logger.info(
                    f"Found {len(policies)} policies for EU AI Act variation: {variation}"
                )
                for policy in policies:
                    logger.info(f"- {policy}")
            else:
                logger.warning(
                    f"No policies found for EU AI Act variation: {variation}"
                )
    ...
    def test_direct_access_vs_get_policies_by_category(self):
        ...
        if eu_direct is None and eu_category is None:
            ...
            logger.warning(
                "Direct access returned None, but get_policies_by_category returned results"
            )
            self.fail("Inconsistent results between methods")
        elif eu_category is None:
            logger.warning(
                "get_policies_by_category returned None, but direct access returned results"
            )
            self.fail("Inconsistent results between methods")
        else:
            ...
            self.assertEqual(
                direct_set,
                category_set,
                "Different policies returned by different methods",
            )
