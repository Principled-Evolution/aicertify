...
def replicate_contract_interactions(
    contract_data: dict, num_replications: int = 25
) -> dict:
    ...
    for i in range(num_replications):
        for interaction in original_interactions:
            ...
            new_timestamp = timestamp.replace(
                microsecond=timestamp.microsecond + i * 1000
            )
            new_interaction["timestamp"] = new_timestamp.isoformat().replace(
                "+00:00", "Z"
            )
...
def main():
    ...
    for app in app_names:
        ...
        command = [
            ...
            app,
            "--folder",
            str(replicated_dir),  # Use the replicated contracts directory
            "--output",
            str(output_file),
            "--category",
            category,
